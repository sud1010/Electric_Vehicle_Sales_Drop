...
with cte as(
  ...
  where 
	...
	and CONVERT(date, sales_transaction_date, 101) >= '2016-10-10'
	and CONVERT(date, sales_transaction_date, 101) <= '2016-10-31'
  group by 
    --grouping the data by sales date to get the total sales--
    CONVERT(date, sales_transaction_date, 101)
) 
select 
  t1.sales_date, 
  t1.num_of_sales, 
  t1.cumul_sales, 
  lag(t1.cumul_sales) over (
      order by t1.sales_date
  ) AS current_sales,
  round(coalesce(((t1.cumul_sales - lag(t1.cumul_sales) over (order by t1.sales_date))* 1.0) / lag(t1.cumul_sales) over (order by t1.sales_date),0)*100,2)
   AS growth_percent
 from cte as t1 
 ...
 with cte as (
  ...
  COUNT(customer_id) as num_of_sales, 
  --counting the no:of customer_id to get total sales of each day--
  sum(COUNT(customer_id)) over (order by CONVERT (date, sales_transaction_date, 101) 
      rows between 6 preceding and current row) as cumul_sales, 
   --using the sum function with over to get the running totals in a column, cumul_sales--
  ...
  where 
    product_id = 8 
	and CONVERT(date, sales_transaction_date, 101) >= '2017-02-15'
	and CONVERT(date, sales_transaction_date, 101) <= '2017-03-08'
  group by 
    CONVERT(date, sales_transaction_date, 101)
) 
select 
  ...
  round(coalesce(((t1.cumul_sales-lag(t1.cumul_sales) over (order by t1.sales_date))* 1.0) 
/ lag(t1.cumul_sales) over (order by t1.sales_date),0)*100,2)
   AS growth_percent
   -- this is the formula for the growth percentage, coalesce is to change the first value of the row to 0 from Null--
...
CONVERT(date, sales_transaction_date) as sales_date,
DATEPART(YY, sales_transaction_date) as year,
DATEPART(MM, sales_transaction_date) as month_of_the_year,
DATEPART(DD, sales_transaction_date) as day,
DATEPART(ww, sales_transaction_date) as week_num
from dbo.sales;
